...
package com.chaos.dmspotongame;
...
public class SpotOnView extends View 
{
   ...
   private long customAnimationTime;//custom animation time for custom spot
   ...
   private static final int CUSTOM_INITIAL_ANIMATION_DURATION=3000;//set the customanimation duration very short
   ...
   private static final int CUSTOM_SPOT_DELAY=1000;
   ...
   public void resetGame()
   {
      ...
      customAnimationTime=CUSTOM_INITIAL_ANIMATION_DURATION;//reset custom time to cutom init time
      ...
      for (int i = 1; i <= INITIAL_SPOTS; ++i)
       ...
       for (int i = 1; i <= INITIAL_SPOTS; ++i)
           spotHandler.postDelayed(addCustomSpotRunnable, i * CUSTOM_SPOT_DELAY);
   ...
   private Runnable addSpotRunnable = new Runnable()
   {
    ...
    // Runnable used to add new spots to the game at the start
    private Runnable addCustomSpotRunnable = new Runnable()
    {
        public void run()
        {
            addNewCustomSpot(); // add a new spot to the game
        } // end method run
    }; // end Runnable
   ...
   public void addNewSpot()
   {
      ...
      spot.setLayoutParams(new RelativeLayout.LayoutParams(
         random.nextInt(SPOT_DIAMETER)+100, random.nextInt(SPOT_DIAMETER)+100));//randomize the spots size
      ...
      spot.animate().x(x2).y(y2).scaleX(SCALE_X).scaleY(SCALE_Y)
         .setDuration(animationTime/(random.nextInt(1)+1)).setListener(//randomize the animation duration, add one to prevent divide by zero!
    ...
    public void addNewCustomSpot()//add new custom spot
    {
        // choose two random coordinates for the starting and ending points
        int x = random.nextInt(viewWidth - SPOT_DIAMETER);
        int y = random.nextInt(viewHeight - SPOT_DIAMETER);
        ...
        spot.setImageResource(random.nextInt(2) == 0 ?
                R.drawable.green_spot : R.drawable.scalednickcage);//add a green spot or nick cage
        spot.setX(x); // set spot's starting x location
        spot.setY(y); // set spot's starting y location
        spot.setOnClickListener( // listens for spot being clicked
                new OnClickListener()
                {
                    public void onClick(View v)
                    {
                        customTouchedSpot(spot); // handle touched spot
                    } // end method onClick
                } // end OnClickListener
        ); // end call to setOnClickListener
        relativeLayout.addView(spot); // add spot to the screen
        ...
        // configure and start spot's animation
        spot.animate().x(x2).y(y2).scaleX(SCALE_X).scaleY(SCALE_Y)
                .setDuration(customAnimationTime).setListener(
                new AnimatorListenerAdapter()
                {
                    @Override
                    public void onAnimationStart(Animator animation)
   ...
   private void touchedSpot(ImageView spot)
   {
    ...
    private void customTouchedSpot(ImageView spot)//when you touch nick cage you get a huge bonus yo!
    {
        relativeLayout.removeView(spot); // remove touched spot from screen
        spots.remove(spot); // remove old spot from list

        ++spotsTouched; // increment the number of spots touched
        score += 10 * level*100; // increment the score and add bonus
